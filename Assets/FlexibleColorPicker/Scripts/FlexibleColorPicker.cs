...
public class FlexibleColorPicker : MonoBehaviour {
    ...
    */
    ...
    public FCP_ExampleScript colorCar;
    ...
    */
    ...
    public Color color
    {
        get
        {
            startingColor = new Color(colorCar.material.color.r, colorCar.material.color.g, colorCar.material.color.b);
        ...
        set
        {
            ...
            // Atualiza o valor do `bufferedColor` e chama métodos para refletir a mudança
            bufferedColor.Set(value);
            UpdateMarkers();    // Atualiza marcadores (indicadores visuais, por exemplo)
            UpdateTextures();   // Atualiza as texturas (caso necessário)
            UpdateHex();        // Atualiza a cor hexadecimal
            typeUpdate = true;  // Define um flag indicando que o tipo foi atualizado
            onColorChange.Invoke(value);  // Dispara o evento de mudança de cor
        }
    }
